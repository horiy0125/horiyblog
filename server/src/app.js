...
bot.on("message", async function (event) {
  ...
  if (event.message.type === "text") {
    console.log(
      bold(
        yellow(
          `event.message.type = ${event.message.type} - ${event.message.text}`,
        ),
      ),
    );
  } else {
  ...
  switch (event.message.type) {
    case "text":
      switch (event.message.text) {
        ...
        case "GetGroupProfile":
          result = await bot.getGroupProfile(
            "Cedb278e86630179f19a395f9f7984a62",
          );
          return event.reply(JSON.stringify(result));

        case "GetRoomMembersCount":
          result = await bot.getRoomMembersCount(
            "Bba70ba25dafbd6a1472c655fe22970c1",
          );
          return event.reply(JSON.stringify(result));

        case "GetGroupMembersCount":
          result = await bot.getGroupMembersCount(
            "Cedb278e86630179f19a395f9f7984a62",
          );
          return event.reply(JSON.stringify(result));

        case "GetIssueLinkToken":
          result = await bot.getIssueLinkToken(
            "U480e5cd6a126659e6b07b95661882ee1",
          );
          return event.reply(JSON.stringify(result));
    ...
    case "image":
      ...
      return event.reply(
        "Nice picture! " + result.toString("hex").substring(0, 32),
      );

    case "video":
      result = await event.message.content();
      return event.reply(
        "Nice video!" + result.toString("hex").substring(0, 32),
      );

    case "audio":
      result = await event.message.content();
      return event.reply(
        "Nice audio!" + result.toString("hex").substring(0, 32),
      );
